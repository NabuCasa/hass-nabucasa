...
async def test_call_check_version_security(
    auth_cloud_mock, mock_cognito, aioclient_mock
):
    """Initialize backend."""
    valid = utcnow() + timedelta(days=1)
    auth_cloud_mock.remote_api_url = "https://test.local/api"
    ...
    assert await remote.check_version_security()
    ...
    assert not await remote.check_version_security()
