...
def test_register(mock_cognito, cloud_mock):
    ...
    auth_api.register(cloud_mock, "email@home-assistant.io", "password")
...
def test_resend_email_confirm(mock_cognito, cloud_mock):
    ...
    auth_api.resend_email_confirm(cloud_mock, "email@home-assistant.io")
...
def test_forgot_password(mock_cognito, cloud_mock):
    """Test starting forgot password flow."""
    auth_api.forgot_password(cloud_mock, "email@home-assistant.io")
...
def test_check_token_writes_new_token_on_refresh(mock_cognito, cloud_mock):
    ...
    auth_api.check_token(cloud_mock)
...
async def test_async_setup(cloud_mock):
    """Test async setup."""
    await auth_api.async_setup(cloud_mock)
    ...
    with patch("random.randint", return_value=0), patch(
        "hass_nabucasa.auth_api.renew_access_token"
    ) as mock_renew:
        await on_connect()
        ...
        assert mock_renew.mock_calls[0][1][0] is cloud_mock
