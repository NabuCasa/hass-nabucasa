...
class Cloud(Generic[_ClientT]):
    ...
    async def _subscription_reconnection_handler(
        ...
        while True:
            ...
            elif subscription_expired_at > (now_as_utc - timedelta(days=1)):
                wait_hours = 3
            elif subscription_expired_at > (now_as_utc - timedelta(days=7)):
                wait_hours = 12
            elif subscription_expired_at > (now_as_utc - timedelta(days=180)):
                wait_hours = 24
            elif subscription_expired_at > (now_as_utc - timedelta(days=400)):
                wait_hours = 96
            else:
                _LOGGER.info(
